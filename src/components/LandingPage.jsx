...
import { auth } from '../firebase/config'; // Adjust this path if your config is elsewhere
...
export default function LandingPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      // This will trigger the Google popup
      await signInWithPopup(auth, provider);
      // Once signed in, the onAuthStateChanged listener in your App.js 
      // or BrazilMap.jsx will detect the user and switch screens automatically.
    } catch (error) {
      ...
      <div style={{ ...styles.mapBackground, opacity: mapReady ? 0.4 : 0 }}>
        ...
        >
          ...
          {COFFEE_ORIGINS.map(origin => (
            <Marker key={origin.id} position={origin.pos} icon={pinIcon} />
          ))}
        </MapContainer>
      </div>
      ...
      <div style={styles.contentWrapper}>
        <main style={styles.heroContent}>
          ...
          <p style={styles.description}>
            ...
            origin data, and brewing metrics in one beautiful, clear interface.
          </p>
...
const styles = {
  ...
  statsRow: { display: 'flex', gap: '40px', marginTop: '48px' },
  stat: { display: 'flex', flexDirection: 'column', alignItems: 'center' },
  statNum: { fontSize: '20px', fontWeight: '800', color: '#8B4513' },
  statDesc: { fontSize: '12px', color: '#A1887F', textTransform: 'uppercase', letterSpacing: '1px' }
};
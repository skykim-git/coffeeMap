  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      >
        ...
        {hiddenCount > 0 && (
          <span style={{
            background: open ? 'rgba(255,255,255,0.25)' : '#5D4037',
      ...
      {open && (
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontSize: '11px', fontWeight: '800', color: '#8D6E63', textTransform: 'uppercase', letterSpacing: '0.8px' }}>
              Visible Columns
            </span>
          ...
          {/* Grouped toggleable columns */}
          {groups.map(group => (
            <div key={group.label}>
              <div style={{
                fontSize: '10px', fontWeight: '700', color: '#BCAAA4',
...
function RawData({ onNavigateToRegion, allRegionDocs = [], beanFilter = null, onClearBeanFilter }) {
  ...
  // Only render the columns that are currently visible
                        ...
                        return (
                          <td key="beans" style={{ ...s.td, ...s.tdBeans }}>
                            <button onClick={() => handleBeanClick(brew)} title={hasRegion ? `View on map: ${regionById[brew.regionRef]?.name}` : 'No region linked'} style={s.beanBtn}>
                              ...
                              <span style={{ ...s.beanBtnIcon, color: hasRegion ? '#5D4037' : '#D7CCC8', opacity: hasRegion ? 1 : 0.4 }}>{hasRegion ? '📍' : '○'}</span>
                            </button>
                          </td>
                        );
...
const s = {
  ...
  filterBanner:      { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 24px', background: '#FFF8E1', borderBottom: '1px solid #FFE082' },
  ...
  // micro buttons inside the popover header
